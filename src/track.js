...
// trackType spec — 레이스 제거됨, vertical 만 (2026-06-05)
...
function buildVerticalTrack({ scene, world, RAPIER }) {
  ...
  const bumpers = [
    ...
    createBumper({ scene, world, RAPIER, x:  3.5, y: 14, mat: bumperMat }),
    createBumper({ scene, world, RAPIER, x: -2.5, y: -4, mat: bumperMat }),
    createBumper({ scene, world, RAPIER, x:  2.5, y: -4, mat: bumperMat }),
  ];
...
function createWindmill({ scene, world, RAPIER, y, mat, reverse = false }) {
  ...
  // hub 제거 — 풍차 중심에 작은 sphere 가 구슬을 받쳐 cluster 유발하던 회귀 (2026-06-05).
  // 시각도 4개 blade 가 자연스러운 풍차 모양. hub 없어도 OK.
